...
const id = getUrlParameterId() //Stocker l'id dans une constante au travers de totu le code
...
function verifyQuantity() {
  ...
  quantity.addEventListener('change', () => {  //Fonction traitant les divers cas de valeurs saisies
    if (quantity.value == Number.isNaN(quantity.value) && quantity.value != "0") {  //Si les données saisies ne sont pas un nombre et suppression de la particularité du cas Zéro du comportement de isNaN
      quantity.value = 0  //Remise de l'input à zéro
      errorMessage.innerText = "Veuillez entrer un nombre s'il vous plait"  //Message d'erreur spécifique
      ...
      quantity.value = 0  //Remise de l'input à zéro
      errorMessage.innerText = "La quantité minimale à ajouter au panier est de un (1)" //Message d'erreur spécifique
      document.querySelector('.item__content__settings__quantity').appendChild(errorMessage)  //Insertion du message d'erreur
    } else if (Number(quantity.value) > 100) {  //Traitement du cas où la quantité demandée est supérieure à 100
      quantity.value = 100  //Mise de l'input à la valeur maximale
      document.getElementById('quantity').setAttribute('value', `${quantity.value}`)
      errorMessage.innerText = "La quantité maximale est de cent (100)" //Message d'erreur sépcifique
      document.querySelector('.item__content__settings__quantity').appendChild(errorMessage)  //Insertion du message d'erreur
...
function createCommandProductInLocalStorage() {
  ...
  button.addEventListener('click', () => {
    ...
    idProduct = new Array(newColorProduct)
...
async function createQueryString() {
  await getProductFromId()
  let colorPicked = getColorOption()
